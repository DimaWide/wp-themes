...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
    ...
    do_action('cr_display_voting_buttons');
    ?>
    ...
    <div id="comment-<?php comment_ID(); ?>" class="<?php echo esc_attr($cr_comment_container_class) . $hide_avatars_wcml_class; ?>">
        ...
        <div class="data-b1-col">
            ...
            if (! $cr_hide_avatars) {
                $get_avtr = get_avatar($comment, apply_filters('cr_review_gravatar_size', '100'), '');
                if ($get_avtr) {
                    echo $get_avtr;
                } else {
                    $hide_avatars_class = ' comment-text-no-avatar';
            ...
            <div class="comment-text<?php echo $hide_avatars_class; ?>">
                ...
                if ($rating && wc_review_ratings_enabled()) {
                    if (0 < $rating) {
                        ...
                        $html_star_rating = '<div class="data-rating">';
                        for ($i = 1; $i <= 5; $i++) {
                            if ($i > 5) {
                                break;
                            }
                            if ($rating >= $i) {
                                $html_star_rating .= '<span>' . file_get_contents(get_stylesheet_directory() . '/img/star.svg', false) . '</span>';
                            } else {
                                $html_star_rating .= '<span class="mod-disable">' . file_get_contents(get_stylesheet_directory() . '/img/star.svg', false) . '</span>';
                            }
                        }
